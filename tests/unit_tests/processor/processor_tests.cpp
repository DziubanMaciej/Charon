...
TEST_F(ProcessorTest, givenConfigWithMatchersWhenCopyActionIsTriggeredThenRequestFileCopy) {
    ...
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "aaa")};
...
TEST_F(ProcessorTest, givenConfigWithMatchersWhenMoveActionIsTriggeredThenRequestFileMove) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createMoveAction(dummyPath2, "aaa")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndCounterUsedWhenCopyActionIsTriggeredThenCheckForFirstFreeFilename) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "###")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndCounterStartUsedWhenCopyActionIsTriggeredThenCheckForFirstFreeFilename) {
    MockFilesystem filesystem{};
    {
        InSequence seq{};
        EXPECT_CALL(filesystem, listFiles(dummyPath2))
            .WillOnce(Return(std::vector<fs::path>{}));
        EXPECT_CALL(filesystem, copy(dummyPath1 / "b.jpg", dummyPath2 / "015.jpg"));
    }

    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "###", 15)};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndCounterStartUsedAndThereAreExistingFilesWhenCopyActionIsTriggeredThenCheckForFirstFreeFilename) {
    MockFilesystem filesystem{};
    {
        InSequence seq{};
        EXPECT_CALL(filesystem, listFiles(dummyPath2))
            .WillOnce(Return(std::vector<fs::path>{
                ...
                dummyPath2 / "010.jpg",
                dummyPath2 / "011.jpg"}));
        EXPECT_CALL(filesystem, copy(dummyPath1 / "b.jpg", dummyPath2 / "009.jpg"));
        EXPECT_CALL(filesystem, listFiles(dummyPath2))
            .WillOnce(Return(std::vector<fs::path>{
                dummyPath2 / "000.jpg",
                dummyPath2 / "001.jpg",
                ...
                dummyPath2 / "010.jpg",
                dummyPath2 / "011.jpg"}));
        EXPECT_CALL(filesystem, copy(dummyPath1 / "c.jpg", dummyPath2 / "012.jpg"));
    }

    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "###", 9)};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndExtensionVariableUsedWhenCopyActionIsTriggeredThenResolveNameProperly) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "a_${extension}_c")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndMultipleActionMatchersWhenEventIsTriggeredThenSelectProperActionMatcher) {
    ...
    ProcessorConfig config = createProcessorConfigWithMatchers({dummyPath1, dummyPath2});
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath1, "abc")};
    config.matchers()->matchers[1].actions = {createCopyAction(dummyPath2, "def")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndExtensionFiltersNotSatisfiedWhenEventIsTriggeredThenSkipIt) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "b")};
    config.matchers()->matchers[0].watchedExtensions = {"png", "jpg"};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndMultipleActionMatchersExtensionFiltersNotSatisfiedWhenEventIsTriggeredThenSelectNextMatcher) {
    ...
    ProcessorConfig config = createProcessorConfigWithMatchers({dummyPath1, dummyPath1, dummyPath1});
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "image")};
    config.matchers()->matchers[0].watchedExtensions = {"png", "jpg"};
    config.matchers()->matchers[1].actions = {createCopyAction(dummyPath2, "song")};
    config.matchers()->matchers[1].watchedExtensions = {"mp3"};
    config.matchers()->matchers[2].actions = {createCopyAction(dummyPath2, "other")};
    config.matchers()->matchers[2].watchedExtensions = {};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersWhenMoveActionIsExecutedThenLogInfo) {
    ...
    config.matchers()->matchers[0].actions = {createMoveAction("b", "${name}")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMathersWhenCopyActionIsExecutedThenLogInfo) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher("a");
    config.matchers()->matchers[0].actions = {createCopyAction("b", "${name}")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndPrintActionWhenEventIsProcessedThenLogInfo) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher("a");
    config.matchers()->matchers[0].actions = {createPrintAction()};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenConfigWithMatchersAndCreateDirectoryEventWhenProcessorIsRunningThenSkipTheEvent) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher("a");
    config.matchers()->matchers[0].actions = {createCopyAction(dummyPath2, "${name}")};
    Processor processor{config, eventQueue, filesystem};
...
TEST_P(ProcessorTestWithDifferentEventsAndActions, givenNonNewEventAndFilesystemActionWhenProcessorIsRunningThenSkipTheAction) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher(dummyPath1);
    config.matchers()->matchers[0].actions = {createActionOfType(actionType)};
    Processor processor{config, eventQueue, filesystem};
...
TEST_F(ProcessorTest, givenCrossDeviceLinkErrorWhenMoveOperationIsTriggerredThenFallbackToCopyPlusRemove) {
    ...
    ProcessorConfig config = createProcessorConfigWithOneMatcher("a");
    config.matchers()->matchers[0].actions = {createMoveAction("b", "dst")};
    Processor processor{config, eventQueue, filesystem};
